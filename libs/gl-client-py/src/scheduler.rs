...
impl Scheduler {
    ...
    fn add_outgoing_webhook(&self, uri: String) -> PyResult<Vec<u8>> {
        convert(exec(async move { self.inner.add_outgoing_webhook(uri).await }))
    }

    fn list_outgoing_webhooks(&self) -> PyResult<Vec<u8>>{
        convert(exec(async move { self.inner.list_outgoing_webhooks().await }))
    }

    fn delete_outgoing_webhooks(&self, webhook_ids: Vec<i64>) -> PyResult<Vec<u8>> {
        convert(exec(async move { self.inner.delete_webhooks(webhook_ids).await }))
    }

    fn rotate_outgoing_webhook_secret(&self, webhook_id: i64) -> PyResult<Vec<u8>> {
        convert(exec(async move { self.inner.rotate_outgoing_webhook_secret(webhook_id).await }))
    }
}
