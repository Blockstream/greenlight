    ...
    R: TlsConfigProvider + RuneProvider + Clone,
{
    ...
    async fn register(
        ...
        match self {
            InnerScheduler::Unauthenticated(u) => u.register(&signer, invite_code).await,
            InnerScheduler::Authenticated(a) => a.register(&signer, invite_code).await,
        }
    }
    ...
    async fn recover(
        ...
        match self {
            InnerScheduler::Unauthenticated(u) => u.recover(&signer).await,
            InnerScheduler::Authenticated(a) => a.recover(&signer).await,
        }
    }

    async fn authenticate(&self, creds: R) -> Result<Self> {
        match self {
            InnerScheduler::Unauthenticated(u) => {
                let inner = u.authenticate(creds).await?;
                Ok(Self::Authenticated(inner))
            }
            InnerScheduler::Authenticated(a) => {
    ...
    R: TlsConfigProvider + RuneProvider + Clone,
{
    ...
    fn authenticated_scheduler(&self) -> Result<&scheduler::Scheduler<R>> {
        match self {
            InnerScheduler::Unauthenticated(_) => {
                Err(anyhow!("Scheduler needs to be authenticated"))
            }
            InnerScheduler::Authenticated(a) => Ok(a),
        }
    }
}
...
pub struct Scheduler {
    node_id: Vec<u8>,
    pub inner: InnerScheduler<PyCredentials, PyCredentials>,
}
...
impl Scheduler {
    ...
    fn new(node_id: Vec<u8>, network: &str, creds: Credentials) -> PyResult<Scheduler> {
        ...
        let inner = match creds.inner {
            crate::credentials::InnerCredentials::Nobody(_) => {
                let scheduler = exec(async move {
                    gl_client::scheduler::Scheduler::with(id, network, creds.inner.clone(), uri)
                        .await
                })
                .map_err(|e| PyValueError::new_err(e.to_string()))?;
                InnerScheduler::Unauthenticated(scheduler)
            }
            crate::credentials::InnerCredentials::Device(_) => {
                let scheduler = exec(async move {
                    gl_client::scheduler::Scheduler::with_authenticated(
                        id,
                        network,
                        creds.inner.clone(),
                        uri,
                ...
                .map_err(|e| PyValueError::new_err(e.to_string()))?;
                InnerScheduler::Authenticated(scheduler)
            }
        };
