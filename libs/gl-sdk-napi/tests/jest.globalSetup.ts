...
export default async function globalSetup(): Promise<void> {
  ...
  const server: ChildProcess = spawn(
    'uv',
    ['run', 'gltestserver', 'run', `--directory=${GLTESTS_DIR}`],
    {
      detached: true,
  ...
  // // Pipe output to console so problems are visible
  // server.stdout?.on('data', (d: Buffer) =>
  //   process.stdout.write(`[gltestserver] ${d}`)
  // );
  server.stderr?.on('data', (d: Buffer) =>
    process.stderr.write(`[gltestserver] ${d}`)
  );
