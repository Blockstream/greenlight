...
impl Signer {
    pub fn new(secret: Vec<u8>, network: Network, tls: TlsConfig) -> Result<Signer> {
        ...
        // restarts
        let persister = Arc::new(crate::persist::MemoryPersister::new("state"));
        ...
        let handler =
            handler::RootHandler::new(network, 0 as u64, Some(sec), Vec::new(), services.clone());
    ...
    pub fn sign_challenge(&self, challenge: Vec<u8>) -> Result<Vec<u8>> {
        ...
        let req = vls_protocol::msgs::SignMessage { message: challenge };
        let response = self
            .handler()
